(function(){
  ...
  function confirmDriver(name){
    return new Promise(resolve=>{
      const modal=document.createElement('div'); modal.className='af-select-modal';
      modal.innerHTML='<div class="af-select-box"><h3>🚗 تأكيد تعيين السائق</h3><div class="af-confirm-text">هل تريد تعيين السائق <span class="af-confirm-name">'+String(name).replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span> لهذا الطلب؟</div><div class="af-select-actions"><button class="af-select-cancel">إلغاء</button><button class="af-select-ok">موافق</button></div></div>';
  ...
  async function choose(type,el){
    ...
    modal.querySelector('.af-select-ok').onclick=async()=>{
      ...
      close(modal);
      if(type==='driver'){
        const confirmed=await confirmDriver(name);
        if(!confirmed)return;
        const m=onclick.match(/assignDriver\s*\(\s*['\"]([^'\"]+)['\"]\s*\)/);
        if(!m){alert('تعذر تحديد الطلب');return}
        try{await api({action:'assign',order_id:m[1],driver_id:value});if(typeof loadOrders==='function')await loadOrders()}catch(e){alert(e.message||'تعذر تعيين السائق')}
        return;
      }
