...
export async function onRequestPost({ request, env }: any) {
  try {
    ...
    const { data: actor } = await admin.from('hr_v2_accounts').select('full_name,role,is_active').eq('id', authData.user.id).maybeSingle();
    if (!actor?.is_active || actor.role !== 'accountant') return json({ error: 'هذا الإشعار مخصص لتقفيلات المحاسبين' }, 403);
    ...
    if (closeoutError || !closeout || closeout.closed_by !== authData.user.id) return json({ error: 'التقفيلة غير موجودة أو غير مملوكة للمحاسب الحالي' }, 404);
    ...
    await Promise.all((subscriptions || []).map(async (row: any) => {
      try {
        const { endpoint, headers, body } = await buildPushHTTPRequest({ privateJWK, subscription: { endpoint: row.endpoint, keys: { p256dh: row.p256dh, auth: row.auth } }, message: { payload, adminContact: 'mailto:noreply@alban-falahi.pages.dev', options: { urgency: 'high', ttl: 86400, topic: 'shift-closeout' } } });
        const response = await fetch(endpoint, { method: 'POST', headers, body });
        if (response.status === 404 || response.status === 410) await admin.from('hr_v2_push_subscriptions').delete().eq('id', row.id);
        else if (response.ok || response.status === 201) sent++;
      } catch {}
    }));
    return json({ ok: true, sent });
  } catch (error: any) {
    return json({ error: error?.message || 'تعذر إرسال إشعار التقفيلة' }, 500);
  }
}
