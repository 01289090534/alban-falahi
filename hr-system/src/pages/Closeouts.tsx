...
export default function Closeouts({ profile }: { profile: Profile }) {
  ...
  const custodyExpenses = num(custody.expense_1);
  const custodyAdditions = 0;
  const remaining = num(custody.previous_cash) - custodyExpenses;
  ...
  async function save() {
    ...
    try {
      ...
      const cp: any = { closeout_id: closeoutId, previous_cash: num(custody.previous_cash), expense_1: num(custody.expense_1), expense_2: 0, expense_3: 0, expense_4: 0, expense_1_note: custody.expense_1_note.trim() || null, expense_2_note: null, expense_3_note: null, expense_4_note: null, cash_addition_1: 0, cash_addition_2: 0, cash_addition_3: 0, cash_addition_4: 0, cash_addition_1_note: null, cash_addition_2_note: null, cash_addition_3_note: null, cash_addition_4_note: null, custody_image_path: custody1, custody_image_path_2: custody2, created_by: u };
  ...
  return <div dir="rtl" className="closeouts-page">
    <style>{`
      ...
      .co-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.co-field{display:flex;flex-direction:column;gap:7px;font-size:12px;font-weight:700;color:#475569}.co-field input,.co-field select,.co-notes{width:100%;box-sizing:border-box;border:1px solid #dbe2ea;border-radius:11px;background:#fff;padding:11px 12px;font:inherit;color:var(--ink);outline:none}.co-field input:focus,.co-field select:focus,.co-notes:focus{border-color:#7abbb4;box-shadow:0 0 0 3px rgba(15,118,110,.08)}
      ...
      .co-custody-top{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px}.co-stat{border-radius:15px;padding:15px;background:var(--soft);border:1px solid var(--line)}.co-stat span{display:block;color:var(--muted);font-size:12px;margin-bottom:5px}.co-stat strong{font-size:20px}.co-stat.main{background:#effaf8;border-color:#ccece7}.co-stat.main strong{color:var(--accent2)}
    ...
    <div className="co-card">
      <div className="co-card-head"><div><div className="co-title"><WalletCards size={19}/> العُهدة</div><div className="co-sub">العهدة السابقة ناقص المصروف المسجل عليها.</div></div></div>
      <div className="co-card-body">
        <div className="co-custody-top">
          <div className="co-stat"><span>العهدة السابقة</span><input className="co-notes" type="number" min="0" value={custody.previous_cash} onChange={e => setC('previous_cash', e.target.value)} placeholder="0"/></div>
          <div className="co-stat"><span>المصروف من العهدة</span><input className="co-notes" type="number" min="0" value={custody.expense_1} onChange={e => setC('expense_1', e.target.value)} placeholder="0"/></div>
          <div className="co-stat main"><span>العهدة الحالية</span><strong>{money(remaining)}</strong></div>
        </div>
        <label className="co-field" style={{marginBottom:14}}><span>بيان المصروف <small>(اختياري)</small></span><input value={custody.expense_1_note} onChange={e => setC('expense_1_note', e.target.value)} placeholder="بيان المصروف من العهدة..." /></label>
        <div className="co-uploads">{fileInput('صورة العهدة 1', custodyFile, setCustodyFile, true)}{fileInput('صورة العهدة 2', custodyFile2, setCustodyFile2, true)}</div>
      </div>
    </div>

    <div className="co-card">
      <div className="co-card-head"><div><div className="co-title"><Camera size={19}/> المستندات والصور</div><div className="co-sub">صورتان لورقة التقفيلة.</div></div></div>
      <div className="co-card-body">
        <div className="co-uploads">{fileInput('صورة ورقة التقفيلة 1', file, setFile, !!editing)}{fileInput('صورة ورقة التقفيلة 2', file2, setFile2, true)}</div>
