...
function addStyle(){
 ...
 .main-cat-box h3{margin:10px 0 5px}.main-cat-box p{color:#786d64}
...
async function renderMainCategories(){
 ...
 try{
  ...
  const cards=groups.map(g=>{
   ...
   return '<div class="main-cat-box">'+(g.image_url?'<img src="'+esc(g.image_url)+'">':'')+'<h3>'+esc(g.name_ar||'بدون اسم')+'</h3><p>'+esc(g.name_en||'')+' — '+count+' فئة</p><button class="btn light" onclick="manageMainCategoryGroup(\''+g.id+'\')">إدارة الفئات</button></div>';
  }).join('');
...
window.manageMainCategoryGroup=manageMainCategoryGroup;
})();